...
- **The ask strip** — along the bottom of the column whenever there is something to ask an agent about: an item, or lines picked out of a job's log. It says what would be sent (*3 lines selected*, *Ask about this issue or pull request*) and carries the *Ask AI* button, which is dim until there is something to send. The button opens a box with a line to type in and the coding-agent CLIs found on this machine, one row each with its version; picking one starts a session on it, and ⏎ in the box sends to the first. Lines are picked in the log the way they are on a diff: a click starts the pick, a ⇧-click stretches it, and the picked lines are tinted in the accent at 18 %.
