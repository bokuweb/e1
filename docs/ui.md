...
- **Asking an agent** starts from a pick, wherever the pick was made. Dragging over text in a rendered comment or body raises a small *Ask AI* chip where the pointer let go — text selection is a window-wide affair in the toolkit, so the column asks for it on mouse-up and anchors the chip in the *window's* coordinates, which is what a mouse event carries. Lines are picked the same way in a job's log and in a file: a click starts the pick, a ⇧-click stretches it, and the picked lines are tinted in the accent at 18 %. In a diff a plain click already means *comment here*, so a pick there is **⌥-click**, and ⌥⇧-click stretches it. All four raise the same chip, and the chip opens the same dialog. Asking about a whole issue or pull, with nothing picked, is the *Ask …* button beside *Open on GitHub*. None of it appears when there is no agent CLI on the machine.
- **The ask itself is a dialog**, not a panel. What is being sent is worth reading before it goes: a box to type in with room for a few lines (⏎ is a newline, ⌘⏎ sends, as on a comment) in the composer's own frame rather than the toolkit's field, whose border and focus ring read as one crooked outline over an opaque panel; then the excerpt, **folded** behind one line saying where it came from and how much of it there is, since the reader picked it a moment ago and knows what it says; then **what goes with it** — the repository and owner, the issue or pull number, the branch and head commit, the labels; for a log the job and workflow run ids, the step that failed and the pull the log was opened from; for a file or a diff the path and the lines — then, when the CLI it is going to takes them, **which model and how much thinking**, each a row of chips whose first is that CLI's own default and puts nothing on the command line (Claude Code takes both, Codex takes both and calls the second a reasoning effort, Cursor names its thinking in its model names and so takes only the first, and the rest get no chips at all); and then one row naming the CLI it will go to, which folds open into the others. Picking one sends and closes; picking a different one makes it the new default, which the settings remember, as they remember the model and the effort per CLI. A failure keeps the dialog open and says so inside it. The dialog is painted opaque: its own default is the window's glass, which is unreadable over a page. The dialog layer is rendered by the window (`Root::render_dialog_layer`), which the toolkit's `Root` does not do on its own.
