...
- **The ask strip** — along the bottom of the column whenever there is something to ask an agent about *and* a CLI to ask: an item, lines picked out of a job's log, or text dragged over in a rendered body. It says what would be sent (*3 lines selected*, *Ask about the selection*) and the button names where it would go (*Ask Claude Code*), dim until there is something to send. With no agent CLI on the machine none of this appears at all — an offer that cannot be taken is worse than no offer. **Dragging over text** in a comment or a body raises a small chip where the pointer let go, which does the same thing: text selection is a window-wide affair in the toolkit, so the column asks for it on mouse-up. The chip is anchored in the *window's* coordinates, because that is what a mouse event carries.
- **The ask itself is a dialog**, not a panel. What is being sent is worth reading before it goes: a box to type in with room for a few lines (⏎ is a newline, ⌘⏎ sends, as on a comment) in the composer's own frame rather than the toolkit's field, whose border and focus ring read as one crooked outline over an opaque panel; then the excerpt, **folded** behind one line saying where it came from and how much of it there is, since the reader picked it a moment ago and knows what it says; then **what goes with it** — the repository and owner, the issue or pull number, the branch and head commit, the labels; for a log the job and workflow run ids, the step that failed and the pull the log was opened from — and then one row naming the CLI it will go to, which folds open into the others. Picking one sends and closes; ⏎ sends to the named one; picking a different one makes it the new default, which the settings remember. The dialog is painted opaque: its own default is the window's glass, which is unreadable over a page. The dialog layer is rendered by the window (`Root::render_dialog_layer`), which the toolkit's `Root` does not do on its own.
