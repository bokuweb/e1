...
**The history** (the *History* chip) is the repository's commits, newest first, one 52 px row each: the subject, then the author, the age and the short hash in mono under it. Down the left runs the **rail**, which is what says the history is not a line. A commit's dot sits in its lane, filled for a plain commit and drawn as a ring for a merge, and every lane with a thread through that row is a hairline in `border.strong`. Lanes are claimed as commits appear and freed when nothing is waiting for them, so a straight history stays in one lane and a merge opens exactly one more; the rail stops at six lanes, because a history wider than that is not read by its picture. The lanes are worked out in `e1_ui::graph`, away from the window, where they are tested. Picking a commit reads it into the right panel.
