...
- **The ask strip** — along the bottom of the column whenever there is something to ask an agent about: an item, or lines picked out of a job's log. It says what would be sent (*3 lines selected*, *Ask about this issue or pull request*) and carries the *Ask AI* button, which is dim until there is something to send. The button names where the ask would go (*Ask Claude Code*) and opens a box with a line to type in and the coding-agent CLIs found on this machine, one row each with its version. The one an ask goes to is marked and tinted; picking another sends there and makes it the new default, which the settings remember. ⏎ in the box sends to the marked one. What is sent carries the **context** on its own: the repository and owner, the issue or pull number, the branch and head commit, the labels — and for a log, the job and workflow run ids, the step that failed, and the pull it was opened from. An agent given those can fetch the rest itself. Lines are picked in the log the way they are on a diff: a click starts the pick, a ⇧-click stretches it, and the picked lines are tinted in the accent at 18 %.
