...
- **Facets** — at the top of the conversation, GitHub's own sidebar shape: *Labels*, *Assignees*, *Projects*, each a heading with a gear, then what the item has (label pills in their colours, avatars with logins, project titles; *None* and *assign yourself* otherwise). The gear opens a picker under the heading: a filter field, then one row per thing the repository — or, for projects, the owner — offers, with a colour dot or avatar, the name, a description, and a check on the rows the item has. A click adds or removes and the picker stays open; the gear closes it. One picker at a time.
- **Merge card** — for an open pull, under the facets: a card in three bands, GitHub's. *All checks have passed / n successful checks* with a green badge (red *Some checks were not successful* with the failing and passing counts; amber *haven't completed yet*), unfolding to the runs with a *Details* link each; *No conflicts with base branch / Merging can be performed automatically* (or the conflict, or *Checking…*); then the green button carrying the method — *Merge pull request*, *Squash and merge*, *Rebase and merge* — with a chevron segment that lists the three methods with GitHub's descriptions and a check on the current one. The first press turns the button into *Confirm merge* beside *Cancel*; the second merges. A draft or a conflicting pull greys the button. The card's border is green when everything is go and red when a check failed.
