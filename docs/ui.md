...
- **A job's log** — when a check's run was picked: the job's name, the repository and job number, then the lines with the runner's clock (`HH:MM:SS`) in the gutter in place of numbers. The runner's markers become colour and weight rather than text: `##[group]` a bold heading on `bg.raised`, `##[command]` in the accent, `##[error]` and `##[warning]` in their status colours on a 12 % tint, `##[endgroup]` dropped. Only GitHub Actions jobs have a log the API will hand over; other checks keep *Details*, which opens the web.
