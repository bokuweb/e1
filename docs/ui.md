...
- **A job's log** — when a check's *Log* was picked: the job's name, the repository and job number, then the log's lines the same way. Only GitHub Actions jobs have one the API will hand over; other checks keep only *Details*, which opens the web.
