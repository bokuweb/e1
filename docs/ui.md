...
- **Anything still running turns.** A check in progress, a step still running, the checks card's own badge while runs are pending, and the sign-in screen's wait all use the toolkit's `Spinner` on the loader glyph. A static loader glyph reads as broken.
