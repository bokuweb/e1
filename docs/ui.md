...
- **Header** — `#number` and the title at 15/500; under it the state glyph and word, the author with their avatar at 18 px, and for a pull `base ← head`, `+adds −dels`, `n files`. Then the actions: *Merge* (filled, only for an open, non-draft pull GitHub does not call unmergeable) — the first press arms it, and the row becomes the three method chips (*Merge commit*, *Squash*, *Rebase*), *Merge now?* and *Cancel* — and *Close* or *Reopen*. While a write is in flight the row says *Working…*; a refusal is GitHub's own words in `status.error` beside the buttons.
- **Facets** — three rows under the actions: *Labels*, *Assignees*, *Projects*, each the muted name, what the item has (label chips, avatars with logins, project titles), and *Edit…*, which opens a picker under the rows listing everything the repository — or, for projects, the owner — offers as chips; a chip the item has is filled, and a click adds or removes. One picker at a time.
