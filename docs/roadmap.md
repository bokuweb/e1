...
The device flow needs an OAuth app with the flow enabled. Its client id is public by design and is built in from `E1_GITHUB_CLIENT_ID`, or read from the same variable at run time; without one the sign-in screen says so and the reader uses the environment or `gh`.
