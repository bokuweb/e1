...
| 2026-09-06 | The logo is drawn with `img`, not `Icon` | `Icon` paints an SVG in one colour, which is right for every mark but this one: the logo carries the accent-to-working gradient the theme is built on, and `img` renders the SVG as it is. |
