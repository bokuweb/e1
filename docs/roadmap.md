...
3. **Local-first, token-only.** A GitHub token is the only requirement. It is discovered from the environment or from `gh`, never stored by this app, and nothing needs an account beyond that.
...
- GitHub Enterprise Server, multiple accounts, OAuth device flow. One token, `api.github.com`.
...
| --- | --- | --- |
| **M0 Shell** | Workspace mirroring Ginka's; tokens, assets, settings, layout persistence; frameless glass window with three resizable columns and draggable header strips; `⌘B`/`⌘⌥B`; en+ja; token discovery; `Scripted` and `E1_DEMO=1` | in progress |
| **M1 Read** | Inbox; the four fixed sections (inbox, my pulls, review requests, assigned); repositories; per-repo pulls and issues, open/closed; detail with markdown body, labels, pull header, comments; open on GitHub; `⌘R` refresh; stale-while-revalidate `Fetch` | in progress, alongside M0 |
| **M2 Review** | Pull files and diffs, checks summary, review decision; mark a notification read; polling the inbox | |
